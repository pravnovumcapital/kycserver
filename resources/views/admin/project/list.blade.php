...
<table class="table table-bordered box table-hover">
    ...
    @foreach($projects as $key=>$project)
      <tr class="center-text">
        <td>{{$project->title}}</td>
        <td>{{str_limit($project->short_description, $limit = 30, $end = '...')}}</td>
        ...
        <td><a href="{{route('admin.project.edit',$project->id)}}"><span><i class="fa fa-edit text-blue"></i></span></a>
          ...
          <a href="#" onclick="deleteRecord('modal-danger','Delete this record?','Are you sure delete this record?','Delete','{{$project->id}}','{{route("admin.project.delete",$project->id)}}')"><span><i class="fa fa-toggle-on text-red"></i></span></td></a> -->
      </tr>
    @endforeach
